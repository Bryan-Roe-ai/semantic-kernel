...
class CopyrightStatusReport:
        ...
        for package_file in package_files[:10]:  # Sample first 10
            try:
                with open(package_file, "r", encoding="utf-8") as f:
                    ...
                    if (
                        "author" in data
                        and isinstance(data["author"], dict)
                        and data["author"].get("name") == self.author
                    ):
                        with_attribution += 1
            except:
                pass
    ...
    def generate_report(self):
        ...
        for filename, info in key_files_status.items():
            ...
            print(
                f"{status_icon} {bryan_icon} {filename:<15} ({size_kb:.1f}KB) - {info['description']}"
            )
        ...
        for file_type, counts in header_counts.items():
            if counts["total"] > 0:
                ...
                print(
                    f"{file_type.capitalize():<12}: {counts['with_header']:>3}/{counts['total']:<3} ({percentage:>5.1f}%) with headers"
                )
        ...
        print(
            f"Sample checked: {package_status['sample_size']}/{package_status['total']} files"
        )
        print(
            f"With attribution: {package_status['with_attribution']}/{package_status['sample_size']}"
        )
        ...
        critical_exists = sum(
            1 for f in critical_files if key_files_status[f]["exists"]
        )
        critical_has_bryan = sum(
            1 for f in critical_files if key_files_status[f]["has_bryan_roe"]
        )

        print(f"Critical files exist: {critical_exists}/{len(critical_files)}")
        print(
            f"Critical files have Bryan Roe attribution: {critical_has_bryan}/{len(critical_files)}"
        )

        total_sample_files = sum(counts["total"] for counts in header_counts.values())
        total_with_headers = sum(
            counts["with_header"] for counts in header_counts.values()
        )

        if total_sample_files > 0:
            header_percentage = (total_with_headers / total_sample_files) * 100
            print(
                f"Key source files with headers: {total_with_headers}/{total_sample_files} ({header_percentage:.1f}%)"
            )
