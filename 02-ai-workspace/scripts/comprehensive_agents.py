...
try:
    from swarm_intelligence_agent import SwarmIntelligenceAgent as SwarmIntelligenceCore
except ImportError:
    SwarmIntelligenceCore = None
...
class SwarmIntelligenceAgent:
    """Wrapper for SwarmIntelligenceCore that implements ImprovementAgent interface."""

    def __init__(self, name: str, workspace_root: str):
        self.name = name
        self.workspace_root = Path(workspace_root)
        self.core = SwarmIntelligenceCore(name, str(workspace_root)) if SwarmIntelligenceCore else None

    async def analyze(self) -> List[ImprovementMetric]:
        """Analyze swarm opportunities and return metrics."""
        if not self.core:
            return []

        try:
            result = self.core.analyze_swarm_opportunities()
            metrics = []

            if result['status'] == 'success':
                swarm_potential = result.get('swarm_potential', 0.7)
                metrics.append(ImprovementMetric(
                    name="swarm_intelligence_potential",
                    value=swarm_potential * 100,
                    target=70.0,
                    direction="higher"
                ))

                opportunities = result.get('opportunities', {})
                collective_ops = opportunities.get('collective_optimization', {}).get('total_opportunities', 0)
                metrics.append(ImprovementMetric(
                    name="collective_optimization_opportunities",
                    value=min(collective_ops * 15, 100),
                    target=60.0,
                    direction="higher"
                ))

                emergence_potential = opportunities.get('emergent_behavior', {}).get('overall_potential', 0.5)
                metrics.append(ImprovementMetric(
                    name="emergence_potential",
                    value=emergence_potential * 100,
                    target=50.0,
                    direction="higher"
                ))

                consensus_readiness = opportunities.get('consensus_building', {}).get('consensus_readiness', 0.3)
                metrics.append(ImprovementMetric(
                    name="consensus_readiness",
                    value=consensus_readiness * 100,
                    target=40.0,
                    direction="higher"
                ))
            ...
            print(f"Error in swarm intelligence analysis: {e}")
            return []

    async def optimize(self, metrics: List[ImprovementMetric]) -> List[ImprovementAction]:
        """Optimize based on swarm analysis and return actions."""
        if not self.core:
            return []
        ...
        try:
            ...
            if result['status'] == 'success':
                implementations = result.get('swarm_implementations', {}).get('implementations', [])
                for implementation in implementations:
                    if implementation.get('success', False):
                        algorithm_name = implementation.get('algorithm', 'swarm_algorithm')
                        actions.append(ImprovementAction(
                            name=f"deploy_{algorithm_name.lower()}",
                            description=f"Deploy {algorithm_name} for collective optimization",
                            estimated_impact=0.8,
                            effort_level="high"
                        ))
            ...
            print(f"Error in swarm intelligence optimization: {e}")
            return []
