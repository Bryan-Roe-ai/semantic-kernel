...
export class DevSkimFixer implements vscode.CodeActionProvider {
    ...
    createMapKeyForDiagnostic(
        diagnostic: vscode.Diagnostic,
        fileName: string
    ): string {
        return `${fileName}: ${diagnostic.message}, ${String(
            diagnostic.code
    ...
    removeFindingsForOtherVersions(fileVersion: FileVersion) {
        var keyNames =
            this.fixMapping.get(fileVersion.fileName)?.keys() ??
            new Array<number>();
    ...
    provideCodeActions(
        ...
        context.diagnostics
            .filter((diagnostic) =>
                String(diagnostic.source).startsWith("DevSkim Language Server")
            )
            .forEach((filteredDiagnostic: vscode.Diagnostic) => {
                const diagnosticKey = this.createMapKeyForDiagnostic(
                    filteredDiagnostic,
                    document.uri.toString().replace(/%3A/g, ":")
                );
                this.fixMapping
                    .get(document.uri.toString().replace(/%3A/g, ":"))
                    ?.get(document.version)
                    ?.forEach((codeFix) => {
                        if (
                            diagnosticKey ==
                            this.createMapKeyForDiagnostic(
                                codeFix.diagnostic,
                                codeFix.fileName
                            )
                        ) {
    ...
    private createFix(
        document: vscode.TextDocument,
        diagnostic: vscode.Diagnostic,
        codeFix: CodeFixMapping
    ): vscode.CodeAction {
        const fix = new vscode.CodeAction(
            codeFix.friendlyString,
            vscode.CodeActionKind.QuickFix
        );
